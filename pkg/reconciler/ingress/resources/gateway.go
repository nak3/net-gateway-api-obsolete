...
const (
	// HTTPRouteNamespaceLabelKey is the label to indicate namespace of HTTPRoute.
	HTTPRouteNamespaceLabelKey = networking.GroupName + "/httprouteNamespace"

	// HTTPRouteVisibilityKey is the label to indicate visibility of HTTPRoute.
	// Unlike `networking.knative.dev/visibility`, this label is added to all HTTPRoute.
...
func MakeGateway(
	...
	return &gwv1alpha1.Gateway{
		...
		Spec: makeGatewaySpec(ing, rule, gatewayConfig),
	}, nil
}
...
func makeGatewaySpec(
	...
	listeners := make([]gwv1alpha1.Listener, 0, len(rule.Hosts))

...
func (c *Reconciler) reconcileHTTPRoute(
	...
	httproute, err := c.httprouteLister.HTTPRoutes(ing.Namespace).Get(resources.HTTPRouteName(rule.Hosts))
	if apierrs.IsNotFound(err) {
		desired, err := resources.MakeHTTPRoute(ctx, ing, rule, gateway)
		if err != nil {
			return nil, err
		}
...
func (c *Reconciler) reconcileGateway(
	...
	gateway, err := c.gatewayLister.Gateways(ns).Get(resources.HTTPRouteName(rule.Hosts))
	if apierrs.IsNotFound(err) {
